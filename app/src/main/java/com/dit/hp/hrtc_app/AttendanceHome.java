...
//        });
        ...
        cardView2.setOnClickListener(v -> {
            ...
            task.addOnSuccessListener(locationSettingsResponse -> {
                // GPS is already ON
                startActivity(new Intent(AttendanceHome.this, AttendanceAuthentication.class));
            });

            task.addOnFailureListener(e -> {
                if (e instanceof ResolvableApiException) {
                    try {
                        ((ResolvableApiException) e).startResolutionForResult(AttendanceHome.this, LOCATION_REQUEST_CODE);
...
//    ##########################################################################################################################
    ...
    // As location enabled then automatically attendance module is opened
    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        super.onActivityResult(requestCode, resultCode, data);

        if (requestCode == 1001 && resultCode == RESULT_OK) {
            // User enabled GPS → Open Attendance screen
            startActivity(new Intent(this, AttendanceAuthentication.class));
        }
    }
