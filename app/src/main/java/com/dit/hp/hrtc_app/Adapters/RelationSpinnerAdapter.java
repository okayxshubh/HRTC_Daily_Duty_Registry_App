...
public class RelationSpinnerAdapter extends ArrayAdapter<RelationPojo> {

    private Context context;
    private List<RelationPojo> values;

    public RelationSpinnerAdapter(Context context, int textViewResourceId, List<RelationPojo> values) {
    ...
    public RelationPojo getItem(int position) {
    ...
    public int getPositionForItem(int id) {
        for (int i = 0; i < values.size(); i++) {
            RelationPojo item = values.get(i);
            // Check if both the name and ID match
            if (item.getRelationId() == id) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        label.setText(values.get(position).getRelationName());
