...
public class CasteSpinnerAdapter extends ArrayAdapter<CastePojo> {

    private Context context;
    private List<CastePojo> values;

    public CasteSpinnerAdapter(Context context, int textViewResourceId, List<CastePojo> values) {
    ...
    public CastePojo getItem(int position) {
    ...
    public int getPositionForItem(String name) {
        for (int i = 0; i < values.size(); i++) {
            CastePojo item = values.get(i);
            // Check if both the name and ID match
            if (item.getCasteName().equalsIgnoreCase(name)) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        label.setText(values.get(position).getCasteName());
